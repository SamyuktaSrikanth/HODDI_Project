...
class HyperAttDDI_SE(nn.Module):
    """
    HyperAttDDI with Side-Effect Conditioned Attention (Exp C).
    ...
    def __init__(self, in_dim=768, emb_dim=256, conv_dim=64, heads=4, d=64, p=0.1):
        ...
        # Module 3: SE-Conditioned Attention Pooling
        # Query is conditioned on BOTH the combination representation and the target side effect
...
def perform_attention_visualization(model, drug_features, data, device, output_dir):
    """
    Evaluates multi-drug combinations across distinct side effects to examine how
    attention weights shift according to the specific adverse event being predicted.
    Generates summary tables and saves visualization artifacts for paper write-up.
    """
    model.eval()
    ...
    # Representative adverse events covering diverse organ systems:
    target_ses = [
    ...
    # Representative clinical combinations:
    # 1. Cardiovascular triple combination: Clopidogrel, Dabigatran, Aspirin
    # 2. Neuro-psychotropic combination: Escitalopram, Clobazam, Pregabalin, Tramadol
    sample_combos = [
        {
    ...
    print("\n=======================================================")
    print("      ATTENTION VISUALIZATION ANALYSIS (Exp C)         ")
    print("=======================================================")
    ...
    with torch.no_grad():
        for combo in sample_combos:
            ...
            # Construct incidence column for this combo
            ...
            logits, alpha = model(drug_features, inc_tensor, se_tensor, return_attention=True)
            probs = torch.sigmoid(logits).cpu().numpy()
            alpha_np = alpha.cpu().numpy()  # (num_ses, num_drugs)
    ...
    viz_path = os.path.join(output_dir, 'attention_visualization.csv')
    viz_df.to_csv(viz_path, index=False)
    print(f"\nAttention visualization table saved to: {viz_path}")
    return viz_df

def main():
    parser = argparse.ArgumentParser(description="HyperAttDDI with SE-Conditioned Attention (Exp C)")
    ...
    parser.add_argument('--output_dir', type=str, default='output_hyperattddi_c', help="Output directory")
    ...
    print("Starting training (HyperAttDDI with SE-Conditioned Attention - Exp C)...\n")
    ...
    for epoch in range(args.epochs):
        ...
        val_res = evaluate_dataset(
            model, drug_features, data['val_inc'], data['val_se'], data['val_lab'], device, batch_size=args.eval_batch_size
        )
        ...
        if val_f1 > max_valid_f1:
            ...
            torch.save(best_weights, os.path.join(args.output_dir, 'best_hyperattddi_exp_c.pt'))
    ...
    test_res = evaluate_dataset(
        model, drug_features, data['test_inc'], data['test_se'], data['test_lab'], device, batch_size=args.eval_batch_size
    )
    
    print("\n--- Final Test Results (HyperAttDDI + SE - Exp C) ---")
    ...
    res_df = pd.DataFrame([{
        'Model': 'HyperAttDDI (Exp C - With SE)',
    ...
    res_df.to_csv(os.path.join(args.output_dir, 'exp_c_results.csv'), index=False)
    print(f"Results saved to: {os.path.join(args.output_dir, 'exp_c_results.csv')}")
    
    # Perform attention visualization
    perform_attention_visualization(model, drug_features, data, device, args.output_dir)
